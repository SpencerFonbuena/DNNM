...
class FCN(nn.Module):
    def __init__(self, data_in, data_out, layers=[128, 256, 512], kss=[8, 5, 3]):
        ...
        self.conv2 = nn.Conv1d(layers[0], layers[0], kss[0], 1, 2)
        self.conv3 = nn.Conv1d(layers[0], layers[0], kss[0], 1, 2)
        self.conv4 = nn.Conv1d(layers[0], layers[0], kss[0], 1, 2)
        self.conv5 = nn.Conv1d(layers[0], layers[1], kss[1], 1, 2)
        self.conv6 = nn.Conv1d(layers[1], layers[1], kss[1], 1, 2)
        self.conv7 = nn.Conv1d(layers[1], layers[1], kss[1], 1, 2)
        self.conv8 = nn.Conv1d(layers[1], layers[2], kss[2], 1, 2)
        self.conv9 = nn.Conv1d(layers[2], layers[2], kss[2], 1, 2)
        self.convout = nn.Conv1d(layers[2], layers[2], kss[2], 1, 1)
        
        self.bn1 = nn.BatchNorm1d(layers[0])
        self.bn2= nn.BatchNorm1d(layers[0])
        self.bn3 = nn.BatchNorm1d(layers[0])
        self.bn4 = nn.BatchNorm1d(layers[0])
