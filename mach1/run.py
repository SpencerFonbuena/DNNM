...
if torch.cuda.is_available():
    path = '/root/GTN/mach1/datasets/AAPL_1hour.txt'
else:
    path = 'models/mach1/datasets/AAPL_1hour.txt'
...
#Load the data
train_dataloader = DataLoader(dataset=train_dataset, batch_size=hp.BATCH_SIZE, shuffle=False, num_workers=2, pin_memory=True ,sampler=samplertrain)
validate_dataloader = DataLoader(dataset=val_dataset, batch_size=hp.BATCH_SIZE, shuffle=False, num_workers=2, pin_memory=True,sampler=samplertrainval)
test_dataloader = DataLoader(dataset=test_dataset, batch_size=hp.BATCH_SIZE, shuffle=False, num_workers=2, pin_memory=True,sampler=samplertest)
    ...
    with torch.no_grad():
        for i, (x, y) in enumerate(dataloader):
            ...
            if i % 1000 == 0:
                ...
                print(torch.cat([max_indices,y.expand(1,hp.BATCH_SIZE).reshape(hp.BATCH_SIZE,1)], dim=-1))
            loss = loss_function(y_pre, y.to(DEVICE))
